...
Number.implement({toFileSize: function (units) {
		if(this == 0) return 0;
...
Locale.define('en-GB', 'uploadManager', {
    BROWSE: 'Browse ...',
    CANCEL: 'Cancel',
	DROP_FILE_HERE: 'Drop files here',
	EMPTY_FILE: 'The selected file is empty',
...
Locale.define('nl-NL', 'uploadManager', {
    BROWSE: 'Browse ...',
    CANCEL: 'Cancel',
	DROP_FILE_HERE: 'Drop files here',
	EMPTY_FILE: 'The selected file is empty',
...
//				else transfer = new HTML5Transfert(opt);
				...
				transfer = new HTML5Transfert(opt);
		...
		Transfert = new Class({
			...
			initialize: function(options) {
				...
				if(options.filetype) this[addEvent]('load',	function (object) { 
					...
					if(this.aborted && !this.message) this.message = Locale[get]('uploadManager.UNAUTHORIZED_FILE_TYPE')
				});
				...
				var first = this.element.getFirst('.upload-progress'),
					...
					this[addEvent]('progress', function (value) {
						...
						if(value == 1) {
							...
							document.getElementById('jform_params_upload').set({value: this.filename});							
						}
					});
			...
			getXHR: function () {
				...
				this.add(xhr.upload, 'progress', function(e) { if (e.lengthComputable) this[fireEvent]('progress', e.loaded / e.total) }).						
					add(xhr, 'load', function() {
						...
						if((json) && (json.size == 0)) this.cancel(Locale[get]('uploadManager.EMPTY_FILE'));
